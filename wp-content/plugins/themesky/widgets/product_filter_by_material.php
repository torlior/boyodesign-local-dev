<?php 
add_action('widgets_init', 'ts_product_filter_by_material_load_widget');

function ts_product_filter_by_material_load_widget()
{
	register_widget('TS_Product_Filter_By_Material_Widget');
}

class TS_Product_Filter_By_Material_Widget extends WP_Widget{

	function __construct(){
		$widgetOps = array('classname' => 'product-filter-by-material', 'description' => esc_html__('Filter products by the material attribute on the shop/product category page. You have to have product attribute with slug "material"', 'themesky'));
		parent::__construct('ts_product_filter_by_material', esc_html__('TS - Product Filter By Material','themesky'), $widgetOps);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( count($terms) > 0 ) {
			...
			foreach ( $terms as $term ) {
				...
				if( $thumb_id ){
					echo wp_get_attachment_image( absint($thumb_id), 'ts_prod_material_thumb', true, array('title'=>$term->name, 'alt'=>$term->name) );	
				}	
				echo '<span class="material-name">'.$term->name.'</span>';				
	...
	protected function get_page_base_url( $taxonomy ) {
		...
		if( !empty($_GET) ){
			$get = $_GET;
			unset($get['filter_material'], $get['query_type_material']);
			if( !empty($get) ){
