...
if( !class_exists('TS_Portfolios') ){
	class TS_Portfolios{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(600, 380);
			$this->add_image_size();
...
if( !class_exists('TS_Team_Members') ){
	class TS_Team_Members{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(480, 480);
			$this->add_image_size();
...
if( !class_exists('TS_Logos') ){
	class TS_Logos{
		...
		function __construct(){
			...
			$logo_height = isset($size_options['size']['height'])?$size_options['size']['height']:70;
...
if( !class_exists('TS_Footer_Blocks') ){
	class TS_Footer_Blocks{
		...
		function __construct(){
			...
			add_action('init', array($this, 'register_post_type'));
			add_action('wp_enqueue_scripts', array($this, 'add_custom_css'), 99999);
		}
		...
		function add_custom_css(){
			...
			$first_footer_area = drile_get_theme_options('ts_first_footer_area');
			$second_footer_area = drile_get_theme_options('ts_second_footer_area');
			
			if( $first_footer_area ){
				$custom_css .= get_post_meta( $first_footer_area, '_wpb_shortcodes_custom_css', true );
			}
			
			if( $second_footer_area ){
				$custom_css .= get_post_meta( $second_footer_area, '_wpb_shortcodes_custom_css', true );
			}
