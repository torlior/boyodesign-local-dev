...
jQuery(document).ready(function($){
	...
	$('.ts-gallery-box .add-image').on('click', function(event){
        ...
        var _states = [new wp.media.controller.Library({
            filterable: 'uploaded',
            title: drile_admin_texts.select_images,
            multiple: true,
            priority:  20
        })];
        ...
        file_frame = wp.media.frames.file_frame = wp.media({
            states: _states,
            button: {
                text: drile_admin_texts.use_images
            }
        });
...
jQuery(document).ready(function($){
	...
	if( add_sidebar_form.length > 0 ){
		...
		$('#ts-add-sidebar').on('click', function(e){
			...
			if( sidebar_name != '' ){
				...
				var data = {
					action: 'drile_add_custom_sidebar'
	...
	if( $('.sidebar-ts-custom-sidebar').length > 0 ){
		...
		$('.sidebar-ts-custom-sidebar .delete-sidebar').on('click', function(){
			...
			var sidebar_nonce = $('#ts_custom_sidebar_nonce').val();
			var ok = confirm( drile_admin_texts.delete_sidebar_confirm );
			if( ok ){
				widget_block.hide();
				var data = {
					action: 'drile_delete_custom_sidebar'
					,sidebar_name: sidebar_name
					,sidebar_nonce: sidebar_nonce
				};
				...
				$.ajax({
					...
					,success : function(response){
						if( response != '' ){
							...
							widget_block.show();
							alert( drile_admin_texts.delete_sidebar_failed );
						}
					}
				});
			}
...
jQuery(document).ready(function($){
	...
	$( document ).on( 'click', '.ts-product-cat-upload-field .upload-button', function( event ) {
		...
		file_frame = wp.media.frames.downloadable_file = wp.media({
			title: drile_admin_texts.choose_an_image,
			button: {
				text: drile_admin_texts.use_image
			},
			multiple: false
		});
